...
def cleanData(data):
	...
	i = 1;
	while i<len(data)-1:
		returnData.append(int(round((data[i-1]+data[i]+data[i+1])/3,0)));
		i+=1;
...
def getOccuranceFrequencies(data, header=False):
	"""
	...
	if not len(pulsesZeros)==0:
		if not header:
			...
			pulseLengthZeros = Counter(pulsesZeros).most_common(1);
			pulseLengthZeros = sorted([int(tup[0]) for tup in pulseLengthZeros]);
	else:
		pulseLengthZeros = [];
	...
	if not len(pulsesOnes)==0:
		if not header:
			...
			pulseLengthOnes = Counter(pulsesOnes).most_common(1);
			pulseLengthOnes = sorted([int(tup[0]) for tup in pulseLengthOnes]);
	else:
		pulseLengthOnes = [];
