...
OUTPUTS: list of 3 characters and 1 integer  [origin, destination, function, length in characters] for example ["A","D","E",1234]
"""
    ...
    while len(header) < 7:
        ...
        if len(morseCache) > 4:
            if morseCache[-4:] == [False,False,False,True]:
                ...
                #print("End of Word")
                #lookingMorse = False
    finalheader = [header[0],header[1],header[2],(int(header[3])*1000 + int(header[4])*100 + int(header[5])*10 + int(header[6]))]
    return finalheader
